...
let FilePreviewService = (() => {
	...
	return class FilePreviewService extends _classSuper {
		static {
			...
			__esDecorate(this, null, _listTree_decorators, {
				kind: "method",
				name: "listTree",
				static: false,
				private: false,
				access: {
					has: (obj) => "listTree" in obj,
					get: (obj) => obj.listTree
				},
				metadata: _metadata
			...
			__esDecorate(this, null, _readTheme_decorators, {
				kind: "method",
				name: "readTheme",
				static: false,
				private: false,
				access: {
					has: (obj) => "readTheme" in obj,
					get: (obj) => obj.readTheme
				},
				metadata: _metadata
			}, null, _instanceExtraInitializers);
			__esDecorate(this, null, _readConfig_decorators, {
				kind: "method",
				name: "readConfig",
				static: false,
				private: false,
				access: {
					has: (obj) => "readConfig" in obj,
					get: (obj) => obj.readConfig
				},
				metadata: _metadata
			}, null, _instanceExtraInitializers);
		...
		static Config = s.object({ maxFileBytes: s.number().step(1).min(1) });
		...
		async walk(target, rel, depth) {
			...
			for (const entry of entries) {
				...
				if (entry.type === "directory") {
					const children = depth > 0 ? await this.walk(entry.target, childRel, depth - 1).catch(() => []) : [];
					nodes.push({
						type: "dir",
						name,
						path: childRel,
				...
				} else if (entry.type === "file") nodes.push(entry.size === void 0 ? {
					type: "file",
					name,
					path: childRel
				} : {
				...
				else nodes.push(entry.size === void 0 ? {
					type: "other",
					name,
					path: childRel
				} : {
		...
		async listTree(request) {
			try {
				...
				return {
					ok: true,
					value: await this.walk(root, "", MAX_DEPTH)
				};
			} catch (error) {
				return {
					ok: false,
		...
		async readFile(request) {
			try {
				...
				if (info.size !== void 0 && info.size > this.maxFileBytes) return {
					ok: false,
					error: {
						code: "too-large",
						path: request.path,
				...
				if (error !== null && typeof error === "object" && error.code === "FS_NOT_TEXT") return {
					ok: false,
					error: {
						code: "not-text",
						path: request.path
		...
		async writeFile(request) {
			try {
				const policy = this.policyFor(request.sessionId);
				const target = await this.ctx.fs.resolve(request.path, { cwd: policy.workspaceRoot });
				await this.ctx.fs.writeText(target, request.content, void 0, void 0, policy);
				return {
					ok: true,
		...
		async readTheme(request) {
			...
			try {
				...
				for (const key of [
					...
					"type",
					"variable"
				]) if (typeof parsed[key] === "string") colors[key] = parsed[key];
			...
			try {
				...
				for (const [mine, theirs] of [
					...
					["type", "types"],
					["variable", "variables"]
				]) if (colors[mine] === void 0 && typeof tcc[theirs] === "string") colors[mine] = tcc[theirs];
				if (bg === null && typeof wcc["editor.background"] === "string") bg = wcc["editor.background"];
				if (fg === null && typeof wcc["editor.foreground"] === "string") fg = wcc["editor.foreground"];
			} catch {}
			return {
				ok: true,
				value: {
...
//#endregion
export { FilePreviewService, FilePreviewService as default };
