 ...
 * @module @deepseek-ai/dsh-client-ui-file-preview/client/render
 */

import type { PreviewThemeColors } from '@deepseek-ai/dsh-file-preview/types'
...
function ltrim(s: string): string {
  let i = 0
  while (i < s.length && (s.charAt(i) === ' ' || s.charAt(i) === TAB)) i++
  return s.slice(i)
}
...
function renderInline(s: string): string {
  ...
  for (let i = 0; i < parts.length; i++) {
    if (i % 2 === 1) out += `<code>${parts[i]}</code>`
    else {
      let p = parts[i] ?? ''
